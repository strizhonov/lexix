...
class RoundedBarChart : BarChart {
    ...
    private fun readRadiusAttr(context: Context, attrs: AttributeSet) {
        val attributes = context.theme.obtainStyledAttributes(attrs, R.styleable.RoundedBarChart, 0, 0)
        try {
            val radius = attributes.getDimensionPixelSize(R.styleable.RoundedBarChart_radius, 0)
            setRadius(radius)
        } finally {
            attributes.recycle()
        }
